...
```
ok    ax/session   9 application(s) own a window: Finder, Yam, …
...
gate polls sixty seconds for the app's window and then exits 2, and on a locked
...
seventeen events per node — which measured 650 ms *per node* against the app's
...
Measured against the app's own 199-node menu-bar tree on an M-series Mac:
**103 Apple events, 2.07 s, 10.4 ms per node**, in one `osascript` invocation.
§7.5's budget — the app's project screen, at least 400 nodes, inside 10 s — is
met with room.
...
real app (`node scripts/record-desktop-tree.mjs --shape ax --screen <name>`), so the mapping, the
