/**
 * `Predicate` → a `CheckResult`, over an accessibility tree (T6.2, LLD §2.3).
 *
...
export interface AxCheckContext {
  /** The nodes of the current snapshot. */
  readonly nodes: readonly AxSnapshotNode[];
  /** The front window's title, which is the nearest thing to a page title. */
  readonly windowTitle: string;
  node(ref: Ref): AxSnapshotNode | undefined;
}
...
export function evaluateAxPredicate(
  predicate: Predicate,
  subject: CheckSubject,
  ref: Ref | undefined,
  context: AxCheckContext,
): CheckResult {
  if (subject === "dialog") {
    /*
     * A macOS sheet or alert *is* an element of the window's tree — `AXSheet`,
     * or an `AXWindow` with an `AXDialog` subrole — rather than a thing the
  ...
  switch (predicate.kind) {
    ...
    case "multiSelect":
      /*
       * macOS publishes `AXSelectedChildren` on a list but System Events does
       * not expose whether *multiple* selection is allowed. Refused rather than
    ...
    case "tag": {
      // The nearest thing a desktop element has to a tag is its `AXRole`.
      const expected = literalValue(predicate.value);
      const actual = node.source.role;
      return negated(result(actual === expected, actual, expected), predicate.negate);
    }
    case "attribute": {
      const expected = literalValue(predicate.value);
