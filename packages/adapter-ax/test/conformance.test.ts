...
describe("the desktop conformance suite against the recorded ADE (T6.2)", () => {
  it("runs every case, and every check holds", async () => {
    /*
     * The bridge maps a press on a tab to the screen it opens, by reading the
    ...
    const bridge = recordedBridge({
      screen: "project",
      onCommand: (command) => {
        if (command.kind !== "action" && command.kind !== "click") return undefined;
    ...
    const report = await runSurfaceConformance({
      ...
      cases: DESKTOP_CASES,
      openSurface: async () => {
        const one = new AxSurface({ processName: "Svatah ADE", bridge });
