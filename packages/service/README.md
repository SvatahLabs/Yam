...
bearer token, generated per process and printed once on stdout. The ADE reads it
...
`openapi.json` is the contract, committed so the ADE's typed client is generated
...
`POST /run` answers with the run id and streams the rest, because the ADE's Run
