...
The headless screen model behind every Svatah builder surface (REQ-ADE-10,
REQ-ADE-13, LLD §13.7).

> The ADE and the terminal cockpit `svatah ui` are two renderers of one headless
...
- **The action registry.** One list behind the ADE's command palette, `svatah
  ui`'s palette, `@svatah/sdk`'s `actions`, and — through each action's `cli`
...
```ts
import { screenById, ACTIONS } from "@svatah/screens";
...
`client` is anything satisfying `ScreenService` — `@svatah/sdk`'s client, the
...
`test/fixtures/fixtures-project.json` is what a real `svatah serve` answered for
...
for: `@svatah/sdk` is generated from the service's description and takes its
