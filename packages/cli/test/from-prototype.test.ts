 ...
 *
 * `evals/migrate/prototype-db` is **synthesised**, and it has to be said plainly: no
 ...
 * importer reads. `scripts/build-prototype-fixture.mjs` regenerates it.
 */
...
const DATABASE = join(ROOT, "evals", "migrate", "prototype-db");
...
function scratch(): string {
  const dir = mkdtempSync(join(tmpdir(), "yam-from-prototype-"));
  projects.push(dir);
  return dir;
}
...
describe("yam migrate --from-prototype (T6.6, REQ-ADE-9)", () => {
  it("writes a project directory from the database's four tables", async () => {
    const project = scratch();
    const result = await cli(["migrate", project, "--from-prototype", DATABASE]);
  ...
  it("maps the prototype's config columns as LLD §13.5 says", async () => {
    const project = scratch();
    await cli(["migrate", project, "--from-prototype", DATABASE]);
  ...
  it("imports the project you name when the database holds several", async () => {
    const project = scratch();
    const result = await cli(["migrate", project, "--from-prototype", DATABASE, "--project", "Smoke"]);
    expect(result.code, result.output).toBe(EXIT.ok);
  ...
  it("refuses a name the database does not hold, and says what it does", async () => {
    const project = scratch();
    const result = await cli(["migrate", project, "--from-prototype", DATABASE, "--project", "Nope"]);
