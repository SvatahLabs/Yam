...
export async function captureCommand(args: ParsedArgs, io: CommandIo): Promise<ExitCode> {
  ...
  try {
    ...
    const relative = join(loaded.config.flows.dir, file);
    const inputs = Object.keys(outcome.inputs);
    io.err(
      `\nwrote ${relative}: "${outcome.story}", ${outcome.steps.length} step${outcome.steps.length === 1 ? "" : "s"}; ` +
        `${saved.written.length} binding${saved.written.length === 1 ? "" : "s"} written` +
        (outcome.unbound.length === 0 ? "" : `; ${outcome.unbound.length} not bound (${outcome.unbound.join(", ")}) — yam record --flow ${relative}`) +
    ...
    if (args.options["json"] !== undefined) {
      io.out(JSON.stringify({ file: relative, story: outcome.story, steps: outcome.steps, inputs: outcome.inputs, bound: outcome.bound, unbound: outcome.unbound }, null, 2));
    }
    return EXIT.ok;
  } finally {
    stdin?.close();
