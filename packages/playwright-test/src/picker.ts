 ...
 *
 * "Record mode (`SVATAH_MODE=record`): if the id is unbound in the current
 ...
 * from".
 *
 * `SVATAH_PICK` is a **test affordance, not a user feature**: it names the
 ...
 * written. Both `SVATAH_PICK` and the `svatahPicks` fixture option go through
...
/**
 * Parse `SVATAH_PICK`: a JSON object of element id → selector.
 *
 * A bare word is read as a test id, so the common case stays short:
 * `SVATAH_PICK='{"login.username-field":"username"}'`.
 */
export function parseProgrammaticPicks(
  ...
  try {
    ...
    throw new Error(
      `SVATAH_PICK is not valid JSON. It is an object of element id to selector, for example ` +
        `{"login.username-field":"username"}.`,
      { cause },
    );
  }
  if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error("SVATAH_PICK must be a JSON object of element id to selector.");
  }
...
const PICKER_SCRIPT = `(id) => {
  const previous = window.__svatahPicker__;
  if (previous) previous.cancel();

  const overlay = document.createElement("div");
  overlay.setAttribute("data-svatah-picker", "overlay");
  ...
  label.textContent = "Svatah is recording \\u2014 click the element for \\u201c" + id + "\\u201d (Esc to cancel)";
  ...
  return new Promise((resolve) => {
    ...
    const finish = (value) => {
      ...
      overlay.remove();
      label.remove();
      delete window.__svatahPicker__;
      resolve(value);
    };
    ...
    const onClick = (event) => {
      ...
      if (!element) return;
      element.setAttribute("data-svatah-picked", id);
      finish(true);
    };
    ...
    window.__svatahPicker__ = { cancel: () => finish(false) };
  });
}`;
 ...
 *
 * The chosen element is stamped with `data-svatah-picked` only long enough for a
...
export async function pickInteractively(
  ...
  return { selector: `[data-svatah-picked="${cssEscape(id)}"]`, source: "human" };
}
...
export async function clearPickerStamp(page: Page): Promise<void> {
  await page
    .evaluate(() => {
      for (const el of Array.from(document.querySelectorAll("[data-svatah-picked]"))) {
        el.removeAttribute("data-svatah-picked");
      }
    })
    .catch(() => undefined);
}
