/**
 * `yam migrate --from-prototype <path>` (T6.6, REQ-ADE-9, LLD §13.5).
 *
 * > Prototype database import (`yam migrate --from-prototype <path>`, P2) maps the
 ...
 */
export function readPrototypeDatabase(path: string): PrototypeDatabase {
...
export interface FromPrototypeOptions {
...
export interface FromPrototypeResult {
 ...
 */
export function extractPrototypeProject(options: FromPrototypeOptions): FromPrototypeResult {
  const db = readPrototypeDatabase(options.source);
  ...
  if (db.project.length > 1 && options.project === undefined) {
    notes.push({
      file: "project",
      line: 0,
      kind: "prototype-multiple-projects",
      message:
  ...
  if (browser !== "" && !["chrome", "chromium", "firefox", "safari", "webkit"].includes(browser)) {
    notes.push({
      file: "config",
      line: 0,
      kind: "prototype-unknown-browser",
      message:
  ...
  writeFileSync(
    ...
    `# Imported from a Yam prototype database (REQ-ADE-9, LLD §13.5).\n` +
  ...
  for (const row of requests) {
    ...
    writeFileSync(
      ...
      `# Imported from a Yam prototype database (REQ-ADE-9).\n${canonicalYaml(request)}`,
      "utf8",
    );
