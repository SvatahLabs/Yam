#!/usr/bin/env node
/**
 * Generate the ADE's service client from `GET /openapi.json` (T3.6, LLD §13.6).
 *
 *   node scripts/generate-ade-client.mjs [--out apps/ade/src/renderer/client.generated.ts]
 *
 * "React renderer with a client generated from `GET /openapi.json`."
 *
 * ## Why generated, and why committed
 *
 * The ADE is the service's reference client (REQ-ADE-1), and a reference client
 ...
 * cannot invent an endpoint, and a route that is renamed breaks the ADE's build
...
const OUT = resolve(
  ROOT,
  outArg > 0 ? process.argv[outArg + 1] : "apps/ade/src/renderer/client.generated.ts",
);
...
lines.push(" *");
lines.push(" * `node scripts/generate-ade-client.mjs` writes this from the service's own");
