#!/usr/bin/env node
/**
 * Screenshot every screen of the packaged app (T9.4, T9.5, T10.1, T10.2).
 *
 *   pnpm app:shoot
 *
 * T10.1 and T10.2 ask for "screenshots of both renderers per screen"; T9.5 asks
 * for the app's to be taken through the AX adapter. This takes both:
 *
 *   * `reports/app-<screen>.png`, one per screen of LLD §13.7 — the renderer's
 *     own pixels, through the DevTools protocol. Always available.
 *   * `reports/app-flows-ax.png` — the *window*, through
 ...
 * project through `YAM_APP_PROJECT` — the same way the desktop gate opens it
 * (LLD §13.6).
 */
 ...
 *
 *   pnpm app:shoot                 # a temporary directory, and it says where
 *   pnpm app:shoot --update        # the committed set under reports/
 *   pnpm app:shoot --out <dir>     # somewhere you name
 *
 * The Phase 10 verification's F6: this rewrote the twelve committed screenshots
 * on *every* run, so a verifier who took a look at the app found twelve
...
const OUT =
  outAt >= 0 && args[outAt + 1] !== undefined
    ...
    : update
      ? join(ROOT, "reports")
      : mkdtempSync(join(tmpdir(), "yam-shots-out-"));
...
function packagedApp() {
  ...
  for (const entry of readdirSync(out)) {
    const mac = join(out, entry, "Yam.app", "Contents", "MacOS", "Yam");
    if (existsSync(mac)) return mac;
    for (const name of ["Yam.exe", "yam", "Yam"]) {
...
const project = mkdtempSync(join(tmpdir(), "yam-shots-"));
...
const desktopApp = spawn(executable, [`--remote-debugging-port=${PORT}`], {
...
// Drained, or a full pipe blocks the application (see apps/desktop/test/shell.spec.ts).
desktopApp.stdout.on("data", () => undefined);
desktopApp.stderr.on("data", () => undefined);
...
try {
  ...
  if (browser === undefined) throw new Error("the app published no DevTools endpoint");
  ...
  for (const [screen, prepare] of SHOTS) {
    ...
    await prepare();
    await sleep(600);
    const path = join(OUT, `app-${screen}.png`);
  ...
  if (process.platform !== "darwin") {
    ...
    if (permission.state !== "granted") {
      ...
      const window = await bridge.window({ process: "Yam", maxNodes: 2_000 });
      const box = window.nodes[0]?.box;
      const path = join(OUT, "app-flows-ax.png");
