#!/usr/bin/env node
/**
 * Put a project into the ADE's Recent list (T11.2, T11.4).
 *
 *   node scripts/seed-ade-recents.mjs [--project evals/fixtures]
 *
 * `evals/self`'s flows open the fixtures project **through the ADE's own Recent
 ...
 * ADE writes when somebody opens a project — so on a machine where nobody has,
...
if (existsSync(path)) {
  try {
    ...
    // A file nobody can read is the defaults, exactly as the ADE treats it.
  }
}
...
process.stdout.write(`${project} is the ADE's first recent project (${path})\n`);
