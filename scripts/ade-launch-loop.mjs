#!/usr/bin/env node
/**
 * Launch and quit the packaged ADE, N times, and say what happened each time
 * (T11.1, T11.2, Draft 2.13 §13.6, P10-F1).
 *
 *   node scripts/ade-launch-loop.mjs [--times 10] [--project evals/fixtures]
 *
 * The Phase 10 verification's F1 is "the packaged ADE runs without a window
 * after its first launches in a session", and its Validate is "the ADE launched
...
/** This checkout's ADE processes, by executable path, so nobody else's count. */
function processIds() {
...
for (let round = 1; round <= times; round += 1) {
  ...
  const leftoverAde = processIds();

  /*
   * The service the ADE spawned, given a moment to finish exiting.
   *
  ...
  rounds.push({
    ...
    quitMs: stopped.ms,
    route: stopped.route,
    leftoverAde: leftoverAde.length,
  ...
  process.stderr.write(
    `round ${String(round).padStart(2)}: window ${created === true ? "created" : "NOT CREATED"}` +
      ...
      `, leftovers ade=${leftoverAde.length} service=${leftoverService.length}\n`,
  );
}
...
const leftovers = rounds.filter((one) => one.leftoverAde > 0 || one.leftoverService > 0).length;
...
if (windows < times || !onScreenAll || leftovers > 0) {
  process.stderr.write("\nThe ADE did not launch cleanly every time; see the rounds above.\n");
  process.exit(1);
}
if (!readableAll) {
  ...
  process.stderr.write(
    "\nEvery launch made a window and put it on screen, and the accessibility API would not " +
      "hand it over. That is a property of the host, not of the ADE, and this is what it " +
      `says:\n\n${doctor.stdout ?? ""}\n`,
  );
  process.exit(3);
}
