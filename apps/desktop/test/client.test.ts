 ...
 * the service breaks the app at run time instead of at build time.
 */
...
const COMMITTED = join(APP_DIR, "src", "renderer", "client.generated.ts");

describe("the committed client is what the generator writes (T3.6)", () => {
  it("regenerating produces the same bytes", () => {
    const out = mkdtempSync(join(tmpdir(), "yam-client-"));
    try {
      const path = join(out, "client.generated.ts");
      execFileSync(
        process.execPath,
        [join(ROOT, "scripts", "generate-app-client.mjs"), "--out", path],
        { cwd: ROOT, stdio: "pipe" },
      );
