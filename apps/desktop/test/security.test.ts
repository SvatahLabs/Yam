...
const read = (...parts: string[]): string => readFileSync(join(APP_DIR, ...parts), "utf8");
...
describe("no renderer source reaches for Node (REQ-ADE-2)", () => {
   ...
   */
  const rendererFiles = [
    ...readdirSync(join(APP_DIR, "src", "renderer"))
      .filter((name) => name.endsWith(".ts") || name.endsWith(".tsx"))
      .map((name) => ["src", "renderer", name]),
    ...readdirSync(join(APP_DIR, "src", "renderer", "shell"))
      .filter((name) => name.endsWith(".ts") || name.endsWith(".tsx"))
...
describe("accessibility, because the app is the desktop conformance target", () => {
