 ...
 *
 * The part of that which is the app's is the *lifecycle*: spawn `yam serve`,
...
/*
 * The interpreter the app would resolve (Draft 2.9 §13.6, T8.1).
 *
 ...
 * `process.execPath` would keep passing on the day the app went back to
...
function fresh(): string {
  userData = mkdtempSync(join(tmpdir(), "yam-userdata-"));
  return userData;
}
...
describe("the lock file (LLD §13.6)", () => {
  ...
  it("treats a live service on another project as stale", async () => {
    ...
    // hand the app somebody else's project.
...
describe("opening the fixture project (T3.6)", () => {
  ...
  it("writes a lock, and adopts rather than starting a second service", async () => {
    ...
    // A reload of the app, or a second window: two services writing to one
...
describe("preferences are the only thing the app stores (REQ-ADE-2)", () => {
