 ...
 *
 * "Editing a flow in the app and compiling from the CLI yields the same
 * `plan.json`; a run started from the app produces the same `runs/<id>` files as
 * the CLI; lint warnings in the editor match `yam lint --json`."
 *
 * The app's screens are React, and rendering them proves nothing about any of
...
beforeAll(async () => {
  ...
  project = mkdtempSync(join(tmpdir(), "yam-parity-"));
...
`,
  ...
  service = await createService({
    ...
    token: TOKEN,
    port: 0,
    // The wiring `yam serve` does, which is the whole reason the app can
...
describe("editing a flow in the app, compiling from the CLI (T3.7)", () => {
  it("yields the same plan hash either way", async () => {
    // The edit the app makes: a real `PUT /flows/:file`, the one the editor calls.
...
describe("a run started from the app (T3.7)", () => {
