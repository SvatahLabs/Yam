...
The desktop app: an Electron client for the local service,
and the service's reference client (REQ-ADE-1, 2).

```bash
pnpm -r build                      # the CLI the app spawns
pnpm --filter @svatah/yam-desktop start    # run it
pnpm --filter @svatah/yam-desktop make     # installers for this platform
pnpm --filter @svatah/yam-desktop smoke    # open the fixture project and quit
```
...
app stores a theme, a window size and a list of recent projects, and nothing else
(REQ-ADE-2).
...
- The renderer's service client is **generated** from `GET /openapi.json`
  (`scripts/generate-app-client.mjs`), so a screen can only call a route the
...
`test/parity.test.ts` is the consequence, checked: an edit through the app and a
compile from the CLI give the same plan hash, a run started through the app
writes the same `runs/<id>` files, and the editor's lint is `yam lint --json`.

## The accessibility variants (`YAM_A11Y_VARIANT`)

The app is the desktop conformance target (REQ-ADE-6), and Draft 2.8 LLD §16
...
```bash
YAM_A11Y=1 YAM_A11Y_VARIANT=1 open -a "…/Yam.app"

# and the gate that drives all three, launching the app once per variant:
...
rival one on the same `runs/`. Quitting stops the service the app started, and a
