...
export function ApiScreen(props: ScreenProps<ApiState>): React.JSX.Element {
   ...
   * editor outside the app, and "a release cannot ship an 'editor' that does
      ...
      <div className="sv-main" style={{ flexDirection: "column" }}>
        <div className="sv-toolbar">
          <Field
            ...
            value={state.source ?? ""}
            placeholder="~/Library/Application Support/yam/db"
            monospace
    ...
    <>
      ...
      <InspectorSection id="inspector-cli" title="The same thing from a terminal">
        <p className="sv-card sv-mono">
          yam migrate {state.root ?? "<dest>"} --from-prototype {state.source ?? "<src>"}
        </p>
      </InspectorSection>
