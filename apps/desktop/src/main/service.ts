/**
 * The local service's lifecycle, from the app's side (T3.6, LLD §13.6).
 *
 ...
 * runs in a terminal and stops with Ctrl-C. It is not enough for the app, which
 * has to survive its own reload and must not leave a second service running on a
 * project that already has one.
 *
 * So the lock file belongs to whoever can guarantee its lifetime: the app writes
...
export interface StartOptions {
   ...
   * in a packaged build that is the app itself with the `RunAsNode` fuse off,
   * so the child is a second APP_DIR that prints no handshake — the whole of P7-F1.
 ...
 *
 * The order matters: adopt first, spawn second. A reload of the app, or a second
...
export async function startOrAdopt(options: StartOptions): Promise<RunningService> {
  ...
  const child = spawn(options.runtime, [options.cli, "serve", options.project, "--port", "0"], {
    ...
    env: {
      ...
      /*
       * And it must not inherit the app's own startup instructions (T8.1).
       *
       * While the spawn was `process.execPath` the child *was* an app, and it
       * read `YAM_APP_SMOKE` from this environment, opened the project, and
       * spawned another: the verifier's "a second APP_DIR instance appears" is one
   ...
   *
   * The app launched by the desktop gate is quit as soon as its window is up,
