/**
 * The app's window-lifecycle log (Draft 2.13 §13.6, P10-F1).
 *
 * > the app logs its window lifecycle to its user-data directory when
 * > `YAM_APP_DEBUG=1`
 *
 * The finding this exists for: a packaged app that "runs without a window".
 ...
 * ## Off unless asked
 *
 * `YAM_APP_DEBUG=1`. A log is a file with a person's project paths in it and
 * the app stores nothing it does not have to (REQ-ADE-2), so it is opt-in, it
...
export const debugLogPath = (userDataDir: string): string =>
  join(userDataDir, "app-debug.log");

/** Is the log on? `YAM_APP_DEBUG=1`, and nothing else. */
 ...
 * `split(" ")`, and a token-shaped string is never written: the app's service
