 ...
 *
 * Everything else the app does goes over HTTP to the local service, with the
 * token this bridge hands out. That is REQ-ADE-1's "the service is the only
 * integration point" taken literally, and it is why the app can claim to show
...
export interface AppBridge {
  ...
  /** A native chooser. The only way a path enters the app. */
   ...
   *
   * `YAM_APP_PROJECT=<dir>` opens a project on ready, before the renderer
...
const bridge: AppBridge = {
  openProject: (directory) => ipcRenderer.invoke("app:openProject", directory) as Promise<ServiceInfo>,
