 ...
 *
 * Four functions on `window.ade` and nothing else (LLD §13.6). Typed here rather
...
declare global {
  interface Window {
    readonly ade: AdeBridge;
  }
}

export function bridge(): AdeBridge {
  if (typeof window === "undefined" || window.ade === undefined) {
    throw new Error(
      "The ADE bridge is missing. This build is meant to run inside Electron with the " +
