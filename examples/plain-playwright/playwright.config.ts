 ...
 *
 * There is nothing Svatah-specific in this file. The whole integration is one
 * dependency and one import in the spec (REQ-PKG-2): tests import `test` from
 * `@svatah/playwright-test` instead of `@playwright/test`.
 *
...
export default defineConfig({
  ...
  use: {
    ...
    // runs headless, including the record pass in CI, which uses SVATAH_PICK.
