...
Status: Draft 2.28 · Date: 2026-09-09 · Owner: Atul Sharma
Companion documents: [hld.md](hld.md) · [lld.md](lld.md) · [tasks.md](tasks.md) · [surface-first spec](surface-first/README.md) · [view layers](view-layers/README.md)
...
Yam is a **deterministic automation runtime with a standard agent surface** whose primary journey is **connect → inspect → act → verify** against a live target, for a person and for an agent, with no project, flow, binding, plan or model credential required for direct control. Saving a reusable automation is optional.

It is built in three layers:

1. **Surface.** A published, agent-native abstraction API (`snapshot` with stable references, `act` by reference, `read`, `check`, session and target lifecycle) implemented by adapters: Playwright, WebDriver BiDi, Appium, OS accessibility (Windows UI Automation, macOS Accessibility, Linux AT-SPI), HTTP, and WebMCP-declared site tools. The surface operates independently of the automation layers.
...
| REQ-REC-13 | `yam record` alone means "I want a new flow, and I am making it by doing it": the browser opens at the application, a person drives, and each click, value entered, choice, key and navigation becomes a sentence of the flow language and each element touched a binding synthesised by the same path a grounded one takes, with provenance `human`/`capture`; a password is never written into a flow (the story declares a `secret` input). Enter at the terminal or closing the browser ends it and writes `flows/<story>.flow`. Binding a flow somebody wrote is asked for by naming it: `yam record --flow <file>` or `--all`. An adapter offers this through the `observe` capability; the Playwright adapter has it, and a click that navigates is held until the element has been bound. Yam.app and the cockpit offer the same two things under their own names: **Record** starts a capture (`POST /capture`, sentences on the stream, Stop writes the flow) and **Bind targets** drives a flow that exists (`POST /record`); one browser, so one session at a time across both (Draft 2.23). | P0 | T |
...
| REQ-BEH-4 | Trajectory compiler: an agent's exploration over the surface (a recorded sequence of surface calls with an optional stated intent per call) compiles into a story draft, a plan, and bindings, written to `proposals/` for review. A call with no intent compiles to a step marked for review rather than failing the file. Replaces the standalone explorer. Intent is user metadata, not required for direct control (Draft 2.25, SF-12). | P2 | D |
...
| ID | Requirement | Pri | Ver |
|---|---|---|---|
| REQ-AGT-1 | CLI: `compile`, `lint`, `record`, `run`, `heal`, `migrate`, `repl`, `eval`, `bindings`, `workflow`, `tool`, `surface` (direct surface control for humans and agents — connect, snapshot, act, read, check, close — with `--json`, operational without a project or flow), with `--json`. | P0 | T |
| REQ-AGT-2 | MCP server exposes both raw surface tools (connect, snapshot, act, read, check, close — operational without a project, flow or intent) and automation operation tools. Default profile foregrounds raw surface operations; automation tools enabled by profile. The automation operations are `yam_compile`, `yam_lint`, `yam_run`, `yam_record`, `yam_heal`, `yam_bindings` and `yam_results`; `yam_record` binds the targets of a flow that exists and refuses the human gateway, having nobody to click, and `yam_heal` proposes unless told to apply (Draft 2.24). Intent is optional user metadata for direct control, never fabricated (Draft 2.25, SF-12). | P0 | T |
...
| REQ-ADE-8 | Agent workbench: a surface explorer that drives any adapter session step by step (intent is optional user metadata, not required for direct control — Draft 2.25, SF-12), and a tool panel that exposes stories over MCP from the app and shows tool invocations with their audit records. | P2 | D |
...
| REQ-ADE-11 | The app is organised as a left rail with Session (default), Automations, Activity and Settings navigation, a centre workspace and a right inspector, with a command palette on ⌘K; every one of the former eleven screens has exactly one home; surface control is directly reachable without visiting Automations or opening a project. The approved mockups under `docs/spec/design/` are the visual reference; supersedes the Flows-first rail of Draft 2.11, and Draft 2.25's Surfaces rail item becomes Session (Draft 2.27, SF-02, SF-16). | P1 | R, D |
...
| REQ-ADE-14 | Direct control and authoring are one destination, not three. **Session** is a single screen over a single broker session with three modes a person switches between without reconnecting: **record** — the person drives the application and Yam observes, writing sentences and synthesising bindings (`REQ-REC-13`); **say** — the person writes one sentence at a time, which Yam grounds against the connected session and performs, appending it to a session flow (`REQ-RUN-11`); **do** — the person selects a control from the snapshot and an action from the published catalogue, and Yam dispatches and verifies it (`SF-11`). All three end in the same artifacts: a flow and bindings that are unverified until reviewed. The mode is visible, switching modes never drops the session, and the element picker is available in every mode because a person clicking the control is a first-class grounder (`REQ-REC-12`). Both renderers present the same three modes under the same names (Draft 2.27). | P0 | T, D |
| REQ-TUI-1 | `yam ui` is a full authoring cockpit in the terminal: a standalone Ink application over the local service, sharing the app's screen model and action registry and owning its own view layer — its own layout, key map, palette behaviour and colour. Parity with the app is *data and actions*: every action the model offers for a screen is reachable from the cockpit, and `--json` streams screen state and audit lines for agents equal to the model's own. The cockpit owns the terminal while it runs and fills it. No tmux dependency (Draft 2.26, superseding Draft 2.11's "numbered panes, the same actions and keys"). | P1 | T, D |
| REQ-TUI-2 | `yam ui --tmux` (alias `yam workspace`) opens a tmux session named for the project with the cockpit, a shell in the project directory, the live audit tail of the current run, and, when `$EDITOR` is set, the editor on the flow the cockpit has open; the four panes share one service through `YAM_SERVICE_URL` and `YAM_SERVICE_TOKEN` so nothing is spawned twice; without tmux the cockpit opens alone and says so; the same session is the process adapter's first target. A convenience, not a release gate, and no P0 acceptance depends on it (Draft 2.20; demoted to P2 by Draft 2.26). | P2 | T, D |
...
| REQ-CLI-11 | A project is configured, not assumed. `yam init` asks a person at a terminal for the project's name, the adapter, where the application runs locally, and any remote endpoints with their kind, and takes the same from flags otherwise; it never points a new project at Yam's sample application, and its first story works against any application. The config carries `endpoints`, a named map of base URL, storage state and environment kind, and `--endpoint <name>` or `YAM_ENDPOINT` selects one for every command, the policy and the app; `yam` shows the endpoint in use (Draft 2.22). | P0 | T |
...
- Draft 2.30 (the DOM walker is shared, owner decision of 2026-09-21): the walker `@svatah/yam-adapter-bidi` evaluates in a page — roles, accessible names, states, boxes and references — moves to `@svatah/yam-page-script`, and `@svatah/yam-adapter-appium` evaluates the same one in a webview. `REQ-ADP-5`'s web half had never worked: the Appium adapter put Chrome's HTML through the Android page-source walker, so every control on a page came back with the role `generic` and the `name` *attribute* for its name, and the emulator gate's first live run found no navigation landmark, no heading and no link on a page full of them. The independence `REQ-ADP-4` asks for is between **BiDi and Playwright** — Playwright snapshots through its own ARIA snapshot, and `snapshot-parity.test.ts` drives the two over the same pages and requires the same answers — and sharing this with Appium costs none of it: Appium was never BiDi's oracle, and a third copy of the accessible-name rules would only be a third place for them to drift. In a webview the adapter also locates in the page rather than through `findElements`, so a reference means one thing (LLD §7.4).
- Draft 2.29 (window chrome is the window manager's, owner decision of 2026-09-21): `Capabilities` gains `windowChrome` — whether an adapter's snapshot carries the window's own close, minimise and zoom controls. `REQ-SURF-3`'s desktop conformance asks for them as evidence that a tree read reaches the window frame and did not silently shrink to the application's content; macOS publishes them as subroles of the window and Windows as the window element's own `WindowPattern`, and **Linux publishes them nowhere**. Under X11 and Wayland alike the decorations belong to the window manager's process, not to the application, so they are absent from its accessibility tree whatever the desktop — and on a bare `Xvfb` with no window manager there are none to own. The flag is what an adapter declares rather than a fact the suite guesses from the platform, which is the same rule `dialogs`, `frames` and `windows` already follow (LLD §2.4); the check is skipped where it is false and still bites everywhere it is true. Found by the first live run of the AT-SPI gate (run 35557004246).
- Draft 2.28 (the view layers are before the publish, owner decisions of 2026-09-09): the view-layer work lands **before** 0.1.0 is published, and there is no release until the owner says the work is done. Because nothing has shipped, the screen ids `surfaces` and `record` are **removed outright** when `REQ-ADE-14`'s Session screen replaces them — no alias, no deprecation window, because a deprecation path for users who do not exist is code with nobody to serve. Agents and tools is **split** rather than kept whole: tool exposure is configuration and belongs under Automations, connected clients and their calls are activity and belong under Activity, and the holder of a target appears on Session; the rail stays at `SF-16`'s four destinations. The app is not deferrable — the model merge removes the screens it renders — so its migration is part of the same work and only its polish may wait. See [view layers](view-layers/README.md) and its [verification](view-layers/verification.md).
- Draft 2.27 (one session, three modes, owner decision of 2026-09-09): `REQ-ADE-14` added at P0. Direct surface control and flow authoring stop being separate destinations: **Session** is one screen over one broker session with **record**, **say** and **do** modes, and every mode ends in a flow with unverified bindings. Superseded assertions: `REQ-ADE-11` and `SF-16`'s rail item becomes **Session** rather than Surfaces; `REQ-REC-13`'s **Record** and **Bind targets** become modes of it rather than destinations of their own, with their capture semantics unchanged; `REQ-RUN-11`'s REPL becomes the **say** mode, and `yam repl` remains its command-line form. The decision is about the product and applies to both renderers, so the app and the cockpit stay one design — see [view layers](view-layers/README.md) for the cockpit's half.
- Draft 2.26 (the terminal owns its view, owner decision of 2026-09-09): `REQ-TUI-1` is restated — `yam ui` shares the screen model and the action registry and owns its view layer, and parity with the app is every action reachable and `--json` equal to the model rather than the same panes and the same keys. Superseded assertion: Draft 2.11's "with numbered panes, the same actions and keys". `REQ-TUI-2` (the tmux workspace) drops from P0 to P2; no P0 acceptance depends on tmux. See [view layers](view-layers/README.md) for requirements TV-01..TV-12, design and implementation tasks.
- Draft 2.25 (surface-first mission, owner decision of 2026-09-07): the primary journey becomes connect → inspect → act → verify against a live target, with no project, flow, binding, plan or model credential required. The surface layer operates independently of the automation layers. Superseded assertions: `REQ-AGT-1` now includes direct surface control commands; `REQ-AGT-2` makes intent optional for direct control and foregrounds raw surface operations in the default MCP profile; `REQ-ADE-8` makes intent optional for the agent workbench; `REQ-ADE-11` makes Surfaces the default navigation, superseding the Flows-first rail; `REQ-BEH-4` makes intent optional for trajectory calls (a call with no intent compiles to a review step); `REQ-CLI-2` adds direct control commands under `surface`. See [surface-first specification](surface-first/README.md) for the full gap analysis, requirements SF-01..SF-23, design and implementation tasks.
