...
- Draft 2.9 (after Phase 7 verification): `REQ-ADE-6` names the packaged ADE as the conformance target and requires it to open a project on every supported host; ADR-4's fine-tune target is recorded as not met for 0.1.0 (measured 86.8 % → 13.2 %) and a Tier 2 corpus is its precondition. Phase 8 (ship) added to the delivery plan; no new requirement ids.
