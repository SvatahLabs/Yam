...
| ~~**The screenshot preview and pixel actions are unbuilt.**~~ **The preview is delivered in step 3; pixel actions are not.** | A **Preview** above the tree draws the surface from `GET /sessions/:session/screenshot.png`, retaken with each snapshot, and a click selects the control whose box is smallest under the pointer (`refAt`) — the tree's own selection, never a click in the application — with the box-to-pixel scale inferred and the caption saying how, though it cannot select where a snapshot carries no boxes (Playwright's AI snapshot) and the lower-assurance pixel mode still does not exist. |
| ~~**Copy CLI and Copy MCP call are unbuilt.**~~ **Delivered in step 3.** | **Copy command** and **Copy MCP call** beside Act generate, from the form's live values, a `yam surface act` line whose arguments go through a quoted heredoc and the matching `surface_act` `tools/call`; the notes say the reference expires with its snapshot, and a password field's value is read from `YAM_SECRET` and marked `--secret` on the command line and is a placeholder listed in `secrets` in the call — never written into either. |
...
| ~~**The connection test does not speak MCP.**~~ **Delivered in step 3.** | `POST /agents/test` starts the server the configuration names — `npx -y @svatah/yam-mcp`, fixed in the service and never read from the request (SF-15) — completes `initialize`, `notifications/initialized` and every page of `tools/list` over its stdio within 60 s, stops its process group, and the panel reports the server, its tool count and the time beside the broker check; it calls no tool, and says so. |
