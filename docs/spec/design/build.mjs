...
for (const f of readdirSync("frag").filter((n) => n.endsWith(".html"))) {
  let body = readFileSync(`frag/${f}`, "utf8");
  body = body.replace(/@@SIDEBAR\((\w+)\)/g, (_, a) => sidebar(a)).replace(/@@TOPBAR\(([^)]*)\)/g, (_, c) => topbar(c)).replace(/@@STATUS\(([^)]*)\)/g, (_, t) => statusbar(t));
