...
Paste the block below into a new Claude Code session opened at the repository root, after `master` has been fast-forwarded to `phase-8` and the Draft 2.10 amendments committed on it. It applies the Phase 8 corrections from `docs/spec/progress/phase-8-verification.md`, then implements Phase 9 (release 0.1.0 and the open P0 items).
...
```
You are continuing the Svatah implementation. Phases 0 through 8 are merged on master, and Phase 8 was verified at 8.9/10 with corrections required. The spec on master is already Draft 2.10, which adds Phase 9 to docs/spec/tasks.md; do not edit the four spec documents. You will apply the corrections, then implement Phase 9.
...
- docs/spec/lld.md
- docs/spec/tasks.md
- docs/spec/progress/phase-8.md and docs/spec/progress/phase-8-verification.md (what exists, and what the verifier measured)
...
F1 The gate can address a still-exiting ADE. Reproduce first if you can: start `pnpm -r test` in the background, run `node scripts/desktop-conformance.mjs --adapter ax`, and watch for `no-window` at variant 1 (one of the verifier's three runs did this). Implement LLD §7.5 as amended: wait for the previous launch's processes to be gone, and choose the process that owns a window (this is T9.1).
F2 The cost line says nothing about load. Record the one-minute load average and CPU count beside the cost; retry an exceeded deadline once and say so in the report (T9.1).
F3 Three unnamed buttons on the ADE's Project screen (the bridge lists three AXButton nodes with no title before "Open a project…"). Name them and make the snapshot case fail on an unnamed interactive control (T9.3).
F4 Progress file. Add a "Post-verification corrections" section to docs/spec/progress/phase-8.md with the verifier's results: the packaged ADE opened by hand, the screenshot taken through the adapter (K1 closed on a terminal with the Screen Recording grant), the gate green twice at load average seven and failed once at eleven with the no-window race, and F1 through F3.

PHASE 9 SCOPE: tasks T9.1 through T9.6 in docs/spec/tasks.md, in order; T9.4 and T9.5 each contain an owner's action you prepare and do not perform; T9.6 only if a Windows host is available. Nothing beyond Phase 9.
...
- Run the desktop gate on a quiet machine and record the load average it reports; a run beside a build is a valid load test for T9.1, not a conformance result.
- T9.2's Tier 2 entries need the pinned local model served by Ollama, which this host has; every new golden entry is reviewed by you and stated as such in the progress file.
- T9.4: write docs/ci.md and stop; do not create accounts, runners, or mirrors. T9.5: do not publish and do not create the tag; the registry script runs in tarball mode until the owner publishes.
...
2. If the spec cannot be followed as written, implement the closest faithful option and record it under Deviations in docs/spec/progress/phase-9.md with the section reference and reason. Do not edit the spec.
...
- docs/spec/progress/phase-9.md with, per correction F1..F4 and per task T9.1..T9.6: status, the exact commands demonstrating each Validate item, observed results, which environment fallback applied; Deviations and Known gaps sections.
...
When finished, print a summary table of F1..F4 and T9.1..T9.6 with status and commit hash, then stop.
```
...
1. Run the six-command contract with no credential on Node 22 and the current LTS; confirm `git diff master..phase-9 -- docs/spec/{requirements,hld,lld,tasks}.md` is empty.
