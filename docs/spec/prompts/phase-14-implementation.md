...
Paste the block below into a new Claude Code session opened at the repository root, after `master` has been fast-forwarded to `phase-12` and its verification committed. It applies the Phase 12 corrections named in `docs/spec/progress/phase-12-verification.md`, then implements Phase 14 (process and terminal).
...
```
You are continuing the Svatah implementation. Phases 0 through 12 are merged on master, and 0.1.0 is released or prepared for the owner's trigger. The spec on master is Draft 2.16 or later; do not edit the four spec documents. You will apply the corrections, then implement Phase 14.
...
- docs/spec/tasks.md (Phase 14, T14.1 through T14.4)
- docs/spec/progress/phase-12.md and docs/spec/progress/phase-12-verification.md

Branching: create branch phase-14 from master. Commit after each item with "P12-F<n>: <title>" or "T14.<n>: <task title>".

PHASE 12 CORRECTIONS (before any Phase 14 task): apply every finding docs/spec/progress/phase-12-verification.md lists under "Findings", in order, each as its own commit, with a self check for each, and add a "Post-verification corrections" section to docs/spec/progress/phase-12.md. The gate must be at 100 percent before T14.1 starts.

PHASE 13 SCOPE: tasks T14.1 through T14.4 in docs/spec/tasks.md, in order. Nothing beyond Phase 14. The target after T14.3 is 45 of 48 on Svatah's side; the ADE's Playwright cases refuse to start while another test-bundle instance is running (Phase 12 verification, F2).
...
- The three REQ-SELF-3 oracles stay external; the target after T14.3 is exactly 45 of 48 on Svatah's side.
...
- docs/spec/progress/phase-13.md with, per correction and per task T14.1..T14.4: status, the exact commands demonstrating each Validate item, observed results, which environment fallback applied; Deviations and Known gaps sections; the parity report embedded.
...
When finished, print a summary table of the corrections and T14.1..T14.4 with status and commit hash, then stop.
```
