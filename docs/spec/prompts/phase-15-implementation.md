...
Paste the block below into a new Claude Code session opened at the repository root, after `master` has been fast-forwarded to `phase-14` and its verification committed. It applies the Phase 14 corrections named in `docs/spec/progress/phase-14-verification.md`, then implements Phase 15 (process and terminal).
...
```
You are continuing the Yam implementation. Phases 0 through 14 are merged on master, and 0.1.0 is released or prepared for the owner's trigger. The spec on master is Draft 2.20 or later; do not edit the four spec documents. You will apply the corrections, then implement Phase 15.
...
- docs/spec/tasks.md (Phase 15, T15.1 through T15.4)
- docs/spec/progress/phase-14.md and docs/spec/progress/phase-14-verification.md

Branching: create branch phase-15 from master. Commit after each item with "P14-F<n>: <title>" or "T15.<n>: <task title>".

PHASE 14 CORRECTIONS (before any Phase 15 task): apply every finding docs/spec/progress/phase-14-verification.md lists under "Findings", in order, each as its own commit, with a self check for each, and add a "Post-verification corrections" section to docs/spec/progress/phase-14.md. The gate must be at 100 percent before T15.1 starts.

PHASE 15 SCOPE: tasks T15.1 through T15.4 in docs/spec/tasks.md, in order. Nothing beyond Phase 15. The target after T15.3 is 45 of 48 on Yam's side; the app's Playwright cases refuse to start while another test-bundle instance is running (Phase 12 verification, F2).
...
- The three REQ-SELF-3 oracles stay external; the target after T15.3 is exactly 45 of 48 on Yam's side.
...
2. If the spec cannot be followed as written, implement the closest faithful option and record it under Deviations in docs/spec/progress/phase-15.md with the section reference and reason. Do not edit the spec.
...
- docs/spec/progress/phase-15.md with, per correction and per task T15.1..T15.4: status, the exact commands demonstrating each Validate item, observed results, which environment fallback applied; Deviations and Known gaps sections; the parity report embedded.
...
- `yam eval self` at 100 percent with Svatah reaching 45 of 48 and the three REQ-SELF-3 oracles the only one-sided rows.
- The verification library's quick start passing against the packed tarball in an empty project.

When finished, print a summary table of the corrections and T15.1..T15.4 with status and commit hash, then stop.
```
...
The verifier will, in a clean worktree of `phase-15`, with the display unlocked:
...
2. Run `yam eval self` and confirm 45 of 48 on Yam's side with only the three oracles one-sided; break one process flow's expectation and confirm the gate bites.
