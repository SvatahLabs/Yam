...
- [~] **TV-T07c · P0 · View/input — The three modes.** *(Partly: see below.)* Requirement: TV-15. The mode strip, one-key switching with the session and the flow preserved (`m`; the digits stay with the regions, and the `TUI-Session` board's `1 record 2 say 3 do` is superseded — see the mode key's own comment in `packages/tui/src/keys.ts`), a mode-scoped key map, the picker in every mode, and **say**'s three answers to an ungroundable phrase. **Done, except one clause.** The modes switch without dropping the session, the key map is mode-scoped, and a sentence typed in **say** never triggers an accelerator — `i` opens the line, letters are letters while it is open, and `esc` gives the keys back.

**Not done: the sentence is not grounded or run.** "A flow started by capture is continued by a written sentence" needs the runtime to ground and perform a step against the *connected broker session*, and the runtime builds its own surface through `createSurface`. That join is the change this specification's own [Not in scope](README.md#not-in-scope-and-why) section names as a runtime change rather than a view-layer one — so the say line holds the sentence and offers the groundings, and nothing pretends to have executed. Depends: TV-T07, TV-T11, and a runtime that takes a broker session.
