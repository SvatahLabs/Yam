# Yam — Low-Level Design

Status: Draft 2.25 · Date: 2026-09-07
Companion documents: [requirements.md](requirements.md) · [hld.md](hld.md) · [tasks.md](tasks.md) · [surface-first spec](surface-first/README.md)
...
- Import boundaries enforced by eslint `import/no-restricted-paths`:
  ...
  - `bindings`, `healer`, `playwright-test`, `bindings-cli` must not import `spec`, `steps`, `compiler`, or `runtime` (module (a) independence, REQ-PKG-1); `cli` may depend on `bindings-cli` and mounts its commands under `yam`.
...
- The DOM walker is one package (Draft 2.30). `@svatah/yam-page-script` holds `walkDocument`, `describeElement`, `locateInPage` and `actionabilityOf` — each serialised to its source and evaluated in the page. `adapter-bidi` evaluates them through `script.callFunction`; `adapter-appium` through `executeScript` when the session is in a webview context, where `getPageSource()` answers with HTML and the native page-source walker cannot read it. `adapter-playwright` does not: it snapshots through Playwright's own ARIA snapshot, which is the independence REQ-ADP-4 measures.

- `windowChrome` (Draft 2.29). Whether the snapshot carries the window's own close, minimise and zoom controls. `ax` publishes them as subroles of the window and `uia` as the window element's `WindowPattern`; `atspi` answers `false`, because on Linux the decorations belong to the window manager's process and are in no application's tree, and a bare `Xvfb` has no window manager to own them. Every non-desktop adapter answers `false`: a page has no window chrome. §16's desktop conformance asks for the three controls only of an adapter that says it has them — the check exists so a tree read cannot silently shrink to the application's content, and on a platform that never publishes them it would be asking the adapter to invent one.
...
Draft 2.17 additions: pattern 32 gains `Exactly one <noun> …` and `… should be unique`; pattern 24 gains a two-element form (`The <a> should be to the left of the <b>`, `… on the same row as …`); pattern 22's captures compare (`{a} should equal {b}`); `app.attach.serviceLock` reads the lock file the app writes and exposes `{app.serviceUrl}` and `{app.serviceToken}` so `Wait for the "<name>" API` can address the application's own service. Desktop `textContains` walks the subtree as the web adapter's does.
...
```ts
import { defineStep, t } from "@svatah/yam-flow";
...
- `bind(id, phrase?)` returns a Playwright `Locator`. Run mode (`YAM_MODE=run`, default): resolver over the store; no model; failure raises `LocatorError` with candidates tried and writes a healer-consumable result line.
- Record mode (`YAM_MODE=record`): if the id is unbound in the current context, calls the recorder's `ground()` (module (b) plugin, §9.2) if installed; otherwise opens an interactive picker in headed mode (click the element) and synthesises the binding with `provenance.model: "human"`. Module (a) alone therefore records without any model.
- Heal mode (`YAM_MODE=heal`): on `LocatorError`, runs relocalization inline, re-tries once, marks the test annotation `healed`, and stages a diff.
...
`yam host generate` (also run implicitly by `yam run --host playwright`) writes `.yam/specs/<flow>.spec.ts`:
...
test.describe("simple.flow", () => {
  ...
  test("Validate logout", async ({ yam }) => { await yam.runStory("Validate logout"); });
});
```

- The `yam` fixture (worker-scoped) creates its own browser context from the worker-scoped `browser` (Playwright's test-scoped `context` is torn down between stories), creates the Playwright adapter over it, loads plan and bindings, and owns the scope for the flow so captures cross stories inside the same worker. `test.describe.configure({ mode: "serial" })` keeps story order; flows map to Playwright projects or files for sharding.
- Failures surface as Playwright assertions with the Yam failure class in the message and `test.info().annotations`; results are additionally written in the Yam schema by a reporter (`@svatah/yam-playwright-test/reporter`).
- Retries: disabled by default; enabled only if the flow's policy is `continue` or the story is `idempotent`.

### 9.2 `bind()` fixture

Provided by `@svatah/yam-playwright-test` (module (a), §6.5); the host re-exports it so a flow project imports one package. The two fixtures share the adapter and store. Model grounding for `bind()` record mode is registered by `@svatah/yam` (`installModelGrounding()`), never by the host, which stays model-free (§1, REQ-RUN-1); a flow project's Playwright config imports it from the CLI package.
...
Replay of a story prefix needs the story's inputs (Draft 2.6). `yam heal --run` accepts `--input name=value` exactly as `run` does and reads `YAM_INPUT_<NAME>`; a run's `summary.json` records the names of the inputs it received, never their values, so an `unreachable` caused by a missing input names it. Secrets are never recorded; the caller supplies them again.
...
`runWorkflow(storyName, inputs, opts)`: builds a one-story run with `behavior: "workflow"`, validates inputs, enables checkpoints and audit, applies policies, returns `{ outputs, summary, runId }`. CLI: `yam workflow run "Book a slot" --input date=2026-09-03 --input user={data.user}` prints outputs as JSON. `--resume <runId> --from <stepId>` supported.
...
**`yam explore` (Draft 2.21, REQ-AGT-5).** One verb over the two halves: it serves the MCP surface on stdio exactly as `@svatah/yam-mcp` does, with the trajectory written to `.yam/explore/<id>/trajectory.jsonl`, and when the client disconnects it compiles the trajectory as `trajectory compile` would and writes the proposal under `proposals/<date>/`, printing the directory and the review notes. An agent host configures `yam explore <project>` as an MCP server command; `--name` names the proposed story. An empty trajectory writes no proposal and says so. `yam explore --trajectory <path.jsonl>` compiles an existing one, for a session recorded by an MCP server or the app's Session screen.
...
`yam serve --project <dir> --port <p> [--token <t>]` starts a Fastify server bound to `127.0.0.1` with a bearer token printed on stdout (the app reads it from the child process). Every handler calls the same functions the CLI calls; no logic lives in the service. The functions are injected through a `ServiceApi` interface so the service imports only `@svatah/yam-schema` (the CLI mounts the service, so an import the other way would be a cycle). `POST /run` validates the supplied inputs against the signatures of the stories the run invokes directly and answers 400 with the missing names before anything starts; `GET /project` includes each story's signature so a client can prompt for inputs.
...
| `GET /plan` | The compiled plan exactly as `yam compile` writes it (steps, tiers, confidence, unbound targets), for the app's Plan screen | `Plan` |
...
| `POST /capture` | Record a flow from what a person does | `{ name? }` → `{ sessionId }`; `capture.started`, a `capture.step` per sentence, `capture.finished` with the flow file and what it bound. The flow is written when the session ends, so `POST /capture/:id/stop` is how one finishes. 409 while any recording session is open — this and `POST /record` share the project's one browser (Draft 2.23, REQ-REC-13) |
| `POST /capture/:id/stop` | End a capture, writing the flow and its bindings | `→ { ok: true }`; `capture.finished` follows |
...
| `POST /surface/:sessionId/snapshot|act|read|check` | Raw surface for the app's picker and for agents | surface wire schemas |
...
The service publishes an OpenAPI description at `GET /openapi.json`, from which the app's typed client is generated; the event stream's message types are the schema package's `ServiceEvent` union.

Prototype database import (`yam migrate --from-prototype <path>`, P2) maps the prototype's electron-db records: `project` → `yam.config.yaml` (name, browser, threadCount → workers, url → baseUrl, takeStepScreenshot → screenshots), `flows` → `flows/<filename>` then v2→v3 migration, `project.locatorFile` JSON → `.locator` → seed bindings, `config.dataFile` JSON → `data.yaml`, `apirequests` → `api/<name>.yaml`. Results and images are not imported.

### 13.6 app client structure (separate repository)

- Electron current LTS, Electron Forge with the Vite plus TypeScript template; `main/` (window, service process lifecycle, project chooser, accessibility flag), `preload/` (typed bridge exposing only `openProject`, `serviceInfo`, `pickFile`, `preferences`), `renderer/` (React; a generated service client; screens listed below). The renderer never has Node access; every capability comes from the service or the bridge. The built main and preload bundles are CommonJS because a sandboxed preload has no ES module loader; sources stay ESM-syntax TypeScript and the renderer is a browser ES module. Electron Forge under pnpm requires `public-hoist-pattern[]=*electron*` in `.npmrc`; no wider hoisting is permitted, and the app pins `vite` to a major whose dependency tree is permissively licensed (REQ-PKG-3).
- Service lifecycle: on project open, locate the bundled CLI (or a configured one), spawn `yam serve --project <dir> --port 0`, read port and token from stdout, health-check `GET /project`, and stop it on project close or app quit. If a service is already running for the directory (lock file with port and token), connect instead.
- The service's runtime (Draft 2.9). The app never runs the CLI with its own binary: the `RunAsNode` fuse is off in a packaged build, so `process.execPath` there is an application, not an interpreter, and the spawn produces a second app that prints no handshake. The runtime is resolved, in order, from `YAM_NODE`, then a `node` on `PATH` of the supported major or newer, then a Node binary shipped beside the CLI under `resources/` when the packager includes one; a packaged app that finds none renders the Project screen's alert naming the three places it looked and opens nothing. `yam surface doctor` and the app's own smoke check report which runtime was chosen. The smoke check (`YAM_APP_SMOKE`) and `scripts/app-smoke.mjs` run against the **packaged** application when one exists under `apps/desktop/out/`, and against the unpackaged build otherwise, and say which.
- Opening a project without a dialog (Draft 2.9). `YAM_APP_PROJECT=<dir>` opens that project on ready through the same `openProject` the Recent list uses; the desktop conformance gate passes the fixtures project this way, so its cases read a project screen rather than the welcome screen.
- Draft 2.10 absorptions. The preload bridge has a one-way `onServiceOpened` beside `onServiceLog`, because a project opened before the renderer exists has no caller to answer; it starts nothing and reads nothing. The packaged app carries a `pnpm deploy` of the CLI under `Resources/yam`, outside the asar, and a Node runtime is never shipped by default. On macOS a GUI application forked from a process outside the user's Aqua session gets no window, so the gate and the smoke check launch the packaged app through LaunchServices (`open -n --env …`). The app's Project screen names every button (LLD §16: names where the tree is thin); an unnamed interactive control in the app is a defect the desktop suite's snapshot case reports.
- Screens and the endpoints and events they render: Project (`GET /project`, `init`); Flow editor (`GET/PUT /flows/:file`, `POST /compile` for inline lint, custom step list from `GET /project`); Plan (`POST /compile` result per story: step, tier, confidence, target status); Run (`POST /run`, `step.result` and `run.summary` events, screenshots by URL, `GET /runs/:id/audit`); Results (`GET /runs`); API client (`POST /api/request`, `GET/PUT /api/:name`); Data (`GET/PUT /data`); Record (`POST /capture`, `capture.step` and `capture.finished` events: the Record button records a flow from what the person does, the screen lists the sentences as they are written, and Stop ends the session; binding a flow that exists is the same screen's **Bind targets**, `POST /record`, `record.decision` and `record.candidates` events, `POST /surface/:session/snapshot` for re-pick; the screen offers the gateway, `anthropic` when the service reports a credential and `fake` for the committed answers, sends it in `POST /record`, labels a fake-gateway session as such, and renders `record.failed` as an alert whose advice is written for the screen, never a CLI flag (Draft 2.7)); Bindings (`GET /bindings`, `POST /bindings/verify`); Heal review (`POST /heal`, `heal.proposal` event, apply through `PUT` of the diff); Surface explorer (`POST /surface/:session/*` with `intent`); Tool panel (`tool serve` control and audit lines).
- Accessibility for the desktop adapters: `app.setAccessibilitySupportEnabled(true)` when launched with `YAM_A11Y=1` or in development builds; every interactive control has a role and an accessible name; screen containers carry landmark roles so `controlPath` candidates are short and stable.
...
The app and the terminal cockpit `yam ui` are two renderers of one headless **screen model**, and both are views over the local service (§13.5) and nothing else. The approved mockups are under `docs/spec/design/` and are the visual reference; this section is the contract.

- **Persona.** Builders: anyone, human or agent, who wants to deterministically automate a surface. There is no separate agent UI. Every screen's state is available as JSON through the service and the SDK (§13.8), every interactive control has a stable accessible name and an id, and `yam ui --json` streams the audit pane to stdout for an agent instead of drawing it.
- **Screen model** (`@svatah/yam-screens`, no DOM, no terminal). A screen is `{ id, title, load(service): State, actions: Action[], keys: Binding[] }`; an `Action` is `{ id, label, run(service, args), availableWhen(state), cli?: string }` and the same list is what the command palette shows, what the SDK exposes as `actions`, and what the CLI has a command for; the repository check asserts the three agree by id. Screens: `flows` (list, editor with lint, plan), `record` (session and decisions), `runs` (list and evidence), `run` (one run, live), `heal` (proposals), `bindings`, `agents` (tool server, invocations, trajectories), `api`, `data`, `explorer`, `import`, `settings`. State is derived from service responses and files only; the review rule of §13.6 stands: no screen holds logic the CLI lacks.
- **Information architecture** (app). Primary navigation: **Surfaces** (default), **Automations**, **Activity**, **Settings**. Surfaces shows available targets, connected sessions and the connect action; a person can connect to a known URL and inspect its controls without visiting Automations or opening a project. Automations holds flows, recording, binding repair, tool publishing and project selection. Activity shows recent surface operations and automation runs. Top bar: project crumb (when a project is open), command palette field, environment and service chips. Centre: the selected screen. Right: a contextual inspector. Status bar: context on the left, keys on the right. Supersedes the Flows-first rail of Draft 2.11 (Draft 2.25, SF-02, SF-16).
...
- **`yam ui`** (`@svatah/yam-tui`, Ink). Full authoring cockpit: four numbered panes (tree, main, inspector, audit), `1–4` focus a pane, `Tab` cycles, `j/k` move, the same actions and keys as the app, the same palette. It opens or adopts a service exactly as the app does. `--json` prints screen state and audit lines as JSON and draws nothing. No tmux dependency; it runs in any terminal.
- **Design system** (`@svatah/yam-ui-tokens` and `@svatah/yam-ui`, React on Radix primitives, MIT). Dark-first with a light theme, IBM Plex Sans and IBM Plex Mono (OFL), one lavender accent for chrome and focus, five status colours (pass, fail, skip, healed, abort/warn/unverified, plus running/info) that never appear without a word or glyph beside them, 13 px text, 28 px controls, 4 px radii, 7 px row padding. Tokens are the seed in `docs/spec/design/base.css`. Monospace is for what the runtime wrote; sans for what the app wrote.
...
- **Screens are tested through the model.** A screen's tests drive the model against the fake service; each renderer has one test per screen that mounts the model and asserts the accessible names and ids the mockup names. The app remains the desktop conformance target (§16).
- Draft 2.12 rules from the Phase 9 verification. The fake service's fixtures are a recording taken against a **copy** of the fixtures project in a temporary directory, never the committed one, so the check does not depend on what else ran there. State carries **timestamps**, never a relative time as text; "20 s ago" is the renderer's, so two loads of one state are equal. The cockpit sizes its panes to the terminal and collapses the inspector below 120 columns rather than clipping it; a capture records the size it was taken at. The app's audit pane renders the call detail the model carries (`locate · booking.book-now-button · testid #0 · ok`), and a toolbar never wraps its buttons: a long title truncates. The sheet's accessibility audit implements every axe-core rule the sheet has ever failed, `landmark-unique` included, and CI fetches axe-core at test time, outside the dependency tree, to run beside it; a rule axe reports and the audit does not is a defect in the audit. `yam ui --capture <ms>` is the product's own flag. `heal.run` belongs to the Run screen and the Flows toolbar shows Record and Run. `POST /runs/:id/stop` cancels a run between steps and the Run screen's Stop action calls it (Phase 10).

### 13.8 SDK and generated clients (Draft 2.11)

- `@svatah/yam-sdk` (TypeScript): a typed client generated from the service's OpenAPI description at build time, plus `subscribe()` over the SSE and WebSocket streams with typed events, plus `actions` (the screen model's action list, runnable out of process). Nothing in it is hand-written that the description already states; a drift between the two fails the build.
- Generated clients for Python (`svatah-yam` on PyPI) and Java (`com.svatah.yam:svatah-yam`) from the same description, with the event stream, published from the same pipeline as the tarballs and versioned with them. They are conformance artifacts of REQ-STD-1's message schemas as much as clients.
- Auth is the bearer token the service prints; the SDK reads `YAM_SERVICE_URL` and `YAM_SERVICE_TOKEN` or a lock file, and never a model credential.

### 13.9 Yam verifies Yam: the self-verification suite and the parity gate (Draft 2.14)

Everything above the surface is verified by Yam's own flows, and the tools a verifier used until now become the second side of a parity gate rather than the only oracle.

- **`evals/self/`** is a Yam project: flows over the Playwright adapter against `apps/sample-web`, over the AX and UIA adapters against the packaged app, over the HTTP adapter against the local service, and over the SDK against `yam ui --json`. Its bindings for the app are seeded from `automationId`s with no model and no recording, and its data holds no secret that is not `${ENV}`.
- **The check catalogue** `evals/self/checks.yaml` lists every check with an id, a sentence, and two implementations: `yam` names a story in the self project; `external` names the independent implementation (a Playwright case over CDP, an osascript probe, a generated-client smoke, a pseudo-terminal capture). Each side answers `pass`, `fail`, or `unreachable` with evidence; `unreachable` says why (no adapter, no grant, no host).
- **`yam eval self [--report reports/self-parity.md]`** runs both sides of every check and compares verdicts per check: agreement (both pass or both fail the same way), disagreement (one passes, the other fails), one-sided (one side unreachable). The report publishes agreement over the checks both sides reach, each side's coverage, wall time per side, and every disagreement and one-sided check by id. **The gate passes only at 100 percent agreement**; a disagreement means one oracle is wrong and is a finding that names both pieces of evidence. One-sided checks are the surfaced shortcomings: every one names the adapter or step Yam lacks, and the list is expected to shrink phase by phase.
- **What stays external on purpose**: the ground-truth keys of the healing eval, axe-core on the component sheet, and the tree-agreement oracle, which reads the app's renderer tree over CDP and compares roles, names, and ids with the AX or UIA snapshot of the same screen. Those three sit below the surface and are what keeps the gate from grading its own homework.
- **Language and adapters gain what the self suite needs.** Desktop adapters take `app.launch` (the executable or bundle, arguments, environment) and `app.quit` (a graceful route, then a signal) in configuration; the session opens by launching when no process of that name owns a window and closes by quitting; the language gains `Quit the app` (pattern 31, `action: "quit"`), which ends the session through the graceful route and fails if the process survives it. The Playwright adapter attaches to an existing Chromium when `YAM_CDP_URL` or `app.attach.cdpUrl` is set, exactly as the BiDi adapter attaches, so a flow can drive the app's renderer. The recorder grounds desktop trees through the same `snapshot()` and the fake gateway gains a desktop case set. Screenshots on macOS remain subject to the Screen Recording grant and are reported as `unreachable` without it, never as a failure.
- **The verification contract changes.** From Phase 11 on, a phase's evidence is `yam eval self` green with its report, plus whatever the report lists as one-sided. The adversarial verifier runs the same command, reads the disagreements and the one-sided list, and probes only what neither side reaches.
- The verification library (Draft 2.16). The catalogue schema, the source runners (a Yam run's results, a Playwright JSON report, a vitest JSON report, a script's exit code and output), the comparison, and the report are `@svatah/yam-verify`, published with the release; `yam eval self --catalogue <file>` runs any project's catalogue, so a third party gates their own application two-sidedly with the same rules. The `process` kind moves the checks that were outside every surface onto Yam's side: the generated-client smoke, the artboard audit, the desktop gate script, the import's filesystem assertion, and the two cockpit checks become flows that run a command and assert on its screen, its exit code, and its files. The ceiling becomes 45 of 48; the three oracles of REQ-SELF-3 stay outside.
- Draft 2.15 rules from the Phase 11 verification. The self project's optional directories (`steps/`, `api/`) are tracked with a placeholder and every script that copies the project tolerates a missing optional directory. The catalogue names an external case by its ancestor titles and its title, and the runner matches on those rather than on a joined string; the catalogue test asserts every external name against the names its source reports. The gate writes its reports outside the tree unless `--update`, as `app:shoot` does. The one-sided list's three gaps are closed by the language: pattern 32, an assertion over a set with a quantifier and a scope (`Every button on this screen should have an id`, `No text on this screen should contain {data.card.number}`, `The rows of the headers table should be …`); pattern 33, `Resize the window to <w> by <h>` with `app.launch.size` as the initial size, performed through the window's size attribute; pattern 19 extended to `Wait for the "<name>" API to answer <json-path> <predicate>` over a service response; `Type` accepts a multi-line value; pattern 11 accepts a chord with no target, sent to the window. The HTTP and SDK sides of the self suite are written, not catalogued. A verifier's Phase 10 finding that the app launched without a window is withdrawn: a locked display makes the accessibility API answer every application's window list with the application element, and the check for "unlocked" must read the lock state.

## 14. Conformance suites (package `conformance`)

- Surface suite: for each sample app page, a script of surface calls with expected snapshot invariants (roles present, names, states), expected `act` effects (URL change, value change, dialog appears), and error types. Runs against any adapter via `yam surface conform --adapter <name>`.
...
- Desktop suite (Draft 2.8): a second case list with the same `ConformanceCase` shape, runner, and report, selected by the surface's `kind`; its cases are the five app flows of §16, the snapshot shape, and a refusal of web-only calls. The web suite's cases begin with `navigate`, which a desktop adapter refuses by design, so the two lists do not merge.

---

## 15. CLI and MCP (package `cli`)

Base URL and storage state precedence (Draft 2.5), applied identically by every command that opens a session (`run`, `record`, `heal`, `bindings verify`, `surface conform`, `eval`, `repl`): the `--base-url` / `--storage-state` flag, then the `YAM_BASE_URL` / `YAM_STORAGE_STATE` environment variable, then `config.app`. A command that opens a session and ignores any of the three is a defect.
...
| `surface doctor` severities | | `ok`, `skip`, `warn` (advisory, exit unchanged: Screen Recording for screenshots, the app's Node runtime when not asked to launch), `FAIL` (exit 1) (Draft 2.10) |
| `scripts/desktop-conformance.mjs --adapter ax\|uia` | `--report` (resolved against the current directory), `--project` | 0 conformant / 1 not / 2 host not ready; polls for the app window up to 60 s (Draft 2.8) |
| `host generate` | `--out` | 0 |
| `trajectory compile <trajectory.jsonl> [dir]` | `--name`, `--out`, `--app` | 0; writes only under `proposals/` (Draft 2.7) |
| `eval self` | `--report`, `--only <check-id>`, `--side yam|external` | 0 at 100 percent agreement / 1 on any disagreement; writes `reports/self-parity.md` (Draft 2.14) |
...
| a target with no binding, at run | "No binding for `the username field` (login.username-field)." | `yam record --all` |
...
MCP server (`@svatah/yam-mcp`, its own installable since PK-02): default profile foregrounds raw surface tools (`surface_connect`, `surface_snapshot`, `surface_act`, `surface_read`, `surface_check`, `surface_close`, each with an optional `intent` for metadata — Draft 2.25, SF-12); automation profile adds operation tools (`compile`, `lint`, `record`, `run`, `heal`, `bindings`, `results`, `workflow`). The `yam explore` trajectory mode requires intent per call to compile into a proposal; direct control does not. Supersedes the intent-required schema of Draft 2.24.
...
- Compiler eval configuration (Draft 2.6): `yam eval compiler` reads `compile.tier2` and `compile.tier3` from the golden project's own committed `yam.config.yaml` (`evals/compiler/project/`), which pins the local model and its digest; `--project` may override it. A tier that is requested but not configured is reported as `not measured` and excluded from the threshold, never scored 0/N.
- Run artifacts (`results.jsonl`, `summary.json`, `audit.jsonl`, screenshots, traces) are committed only under `evals/conformance/` and `reports/`. Every project directory ignores `runs/`, `.yam/`, and any absolute-path echo such as `var/`; a repository check enforces it.
...
- Draft 2.25 (surface-first mission, owner decision of 2026-09-07): `packages/surface-control` added as a shared operation dispatcher with session lifecycle and an operation catalogue generating CLI, MCP and service schemas — no compiler, recorder, gateway or service imports (§1, HLD §12); desktop navigation becomes Surfaces (default), Automations, Activity, Settings — Surfaces shows targets and sessions with no project required (§13.7); MCP default profile foregrounds raw surface tools with optional intent, automation tools enabled by profile (§15); intent is optional for direct control and never fabricated (SF-12); trajectory mode still requires intent. Superseded assertions: the Flows-first rail of §13.7 Draft 2.11; the intent-required MCP schema of §15 Draft 2.24; the project-required surface dispatch of §13.5.
- Draft 2.24 (the cockpit's keys and the workspace's size): the Flows screen binds `R` to `capture.start` and `b` to `record.start`, which Draft 2.23's split left pointing at one action under the other's name (§13.7); the cockpit reads a `^x` terminal binding, so the Data screen's `^s` save is reachable rather than declared and dead; `yam workspace` creates its session at the terminal's size and gives the cockpit 60 percent of the horizontal split, because tmux's default 80×24 left the cockpit pane at 40 columns, under its own 60-column minimum, with the inspector collapsed (§15.1). A screen's key table is checked against the action registry, which is what nothing did.
- Draft 2.24 (the MCP operations): the MCP server's operation tools are `yam_compile`, `yam_lint`, `yam_run`, `yam_record`, `yam_heal`, `yam_bindings`, `yam_results` (§13.4, §15). `yam_record` and `yam_heal` call `serviceRecord` and `serviceHeal`, the same entry points the local service calls, so an agent's binding and the app's are one implementation; `yam_record` takes `gateway: anthropic | fake` and never `human`, because an MCP session has nobody to click, and `yam_heal` defaults to `apply: false`. A test asserts the published list exactly and against the table in `docs/mcp.md`, the drift having gone unnoticed since Phase 4.
- Draft 2.23 (record is capture): the service gains `POST /capture` and `POST /capture/:id/stop` (§13.5), sharing one session slot with `POST /record` because both open the project's browser, and four `capture.*` events (§13.8); `ServiceApi.capture` runs through the same `captureIntoProject` the command line uses; the app's **Record** button captures (`capture.start`, key R) and **Bind targets** (`record.start`, key B) binds a flow that exists, both landing on the Record screen, which lists the sentences while a capture runs and says where the flow was written (§13.6, §13.7); the cockpit shows the same. Also: the surface contract gains the optional `observe(handler, {signal})` and the `observe` capability (§2.1, §2.4; eleven flags); an `ObservedEvent` is click, type (with `secret`), select, check, press, navigate (with `typed` when the adapter knows) or closed. The Playwright adapter's in-page observer (`packages/adapter-playwright/src/observer.ts`) stamps the element and calls an exposed binding; a click on a link or submit button, and a form's submit, are cancelled, the observations in flight awaited, and re-dispatched marked to pass, so the element is bound on the page it was on; the adapter also holds a document request while observations are pending. `YAM_OBSERVE` (a JSON array of actions) is the scripted person. The recorder's `capture()` (`packages/recorder/src/capture.ts`) turns events into sentences — `Click the sign in button`, `Type "…" into the username field`, `Type {input.password} into the password field` with `inputs: password: secret`, `Select`, `Check`/`Uncheck`, `Press`, `Go to` for a navigation the person typed — binds each element once through `entryFor` (provenance `human`/`capture`), ends with `The URL should contain`, and renders the flow file. `yam record` without `--flow`, `--all`, `--story`, `--rebind`, `--gateway` or `--force-production` is capture (`packages/cli/src/commands/capture.ts`): headed, asks the story's name at a terminal, Enter ends it, writes `flows/<story>.flow` and saves the store; without a person at a terminal it refuses naming `--flow` (`cannot-drive`); an adapter without `observe` is refused (`cannot-observe`). The front door names `yam record --all` for unbound targets; §15.1's record line and the catalogue row changed accordingly.
- Draft 2.22 (endpoints): the config gains `endpoints: { <name>: { baseUrl, storageState?, environment? } }` and `endpoint` (the default name) (§3.5); `loadConfig` applies the endpoint `YAM_ENDPOINT` or `endpoint` names to `app.baseUrl`, `app.storageState` and `environment` before any command sees the config, and refuses an unknown name naming the known ones; `--endpoint <name>` on every command sets `YAM_ENDPOINT` before dispatch (§15); `yam init` asks for the local endpoint and the remote ones, or takes `--name`, `--adapter`, `--url`, `--endpoint name=url[@kind]` and `--yes`; its first story is `Go to "/"` and a URL check (§15.1); the front door prints an `app` line with the base URL, the environment kind, the endpoint in use and the others. The example `init` writes carries a marker line; `yam record` names a stopped step's story as the example when it is one, and names a 4xx/5xx navigation as a page the application does not have (`packages/cli/src/scaffold.ts`); `init --force` retires the pre-2.22 example it wrote (`flows/sign-in.flow` under the old header); the human-gateway preamble says record performs the flow as written.
...
- Draft 2.16 (process and terminal, owner decision of 2026-09-06): the `process` surface kind and `adapter-process` (§2.4); patterns 34–38 and the multi-line `Type` in the pattern table (§4.2); `@svatah/yam-verify` and `yam eval self --catalogue` for third parties, and the six outside-surface checks moved to Yam's side (§13.9); Phase 13 added after the release.
...
- Draft 2.14 (Yam verifies Yam, owner decision of 2026-09-05): the self-verification suite `evals/self`, the check catalogue, `yam eval self` and the two-sided parity gate at 100 percent agreement, the three oracles kept external, `app.launch`/`app.quit` and `Quit the app` (pattern 31), the Playwright adapter's CDP attach, desktop grounding (§13.9, §4.2, §7.1, §7.5); the verification contract becomes the gate plus its one-sided list.
- Draft 2.13 (after Phase 10 verification): app window-lifecycle debug log and a graceful quit route for the gate, the id rule exempts standard window chrome, default row selection, toolbar title budget, one-line selects, `availableWhen` on buttons, no secret-derived facts, screenshots outside the tree, a distinct bundle name for the test build, the flow editor and API request editing (§13.7); a timed-out probe is "could not tell" (§7.5).
- Draft 2.12 (after Phase 9 verification): fixtures recorded against a temporary copy, timestamps not relative text in state, cockpit panes sized to the terminal, audit detail and toolbar rules in the app, the accessibility audit covers every axe rule with axe run beside it in CI, `--capture`, `heal.run` on the Run screen, `POST /runs/:id/stop` (§13.7); the `ax/session` doctor check (§7.5).
- Draft 2.11 (builder surfaces, after the owner's design review): one headless screen model rendered by the app and by `yam ui`, the sidebar-workspace-inspector architecture, the command palette with action parity across palette, TUI, SDK and CLI, the design system on Radix with the approved tokens, the accessibility contract, `--json` for agents (§13.7); `@svatah/yam-sdk` generated from the OpenAPI description with Python and Java clients (§13.8); the approved mockups under `docs/spec/design/`.
- Draft 2.10 (after Phase 8 verification): the AX window read is an in-process `AXUIElementCopyAttributeValue` helper and the Apple-event form is struck; the cost line records accessibility calls, load average and CPU count; the gate retries a deadline once, waits for the previous instance to exit, and the bridge addresses the process that owns a window (§7.5); `onServiceOpened`, the CLI deploy under `Resources/yam`, launch through LaunchServices, named controls on the Project screen (§13.6); `eval finetune corpus` and the corpus-only export, `surface doctor` severities (§15).
- Draft 2.9 (after Phase 7 verification): the app never runs the CLI with its own binary, resolves a Node runtime and says which, opens `YAM_APP_PROJECT` on ready, and its smoke check runs against the packaged application (§13.6); the §7.5 budget is satisfied only by a measured snapshot of the project screen, whole-window reads or a native helper, the caller's deadline honoured, empty cases skipped and healing cases run at their own variant (§7.5); `dialog` steps arm the next dialog, with `W_DIALOG_UNARMED`, `W_DIALOG_NEVER_OPENED`, and an audit line for an unarmed default (§3.2, §4.2).
- Draft 2.8 (after Phase 6 verification): `dialog` args are `{ action, text? }` and adapters read `action` (§3.2); desktop bridges over `osascript`/PowerShell permitted with bulk reads and a 400-node, 10 s budget, `automationId` from `AXDOMIdentifier`, scoped screenshots (§7.5); `POST /migrate` into the open project (§13.5); the runtime suite validates a foreign runtime's artifacts against the schemas, the fixture is a projection, the desktop suite is a second case list (§14); `surface doctor`, `eval finetune export`, and the desktop gate script in the command table (§15); desktop healing cases through app variants (§16); `pnpm -r typecheck` joins the verification contract (§16).
...
- Draft 2.1: local service (§13.5) with OpenAPI description; new app client structure (§13.6) built to the vision with the prototype as blueprint; prototype database import at P2; Electron accessibility note (§7.5); the app replaces the sample desktop app (§16).
