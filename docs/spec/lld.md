...
| `GET /plan` | The compiled plan exactly as `yam compile` writes it (steps, tiers, confidence, unbound targets), for the ADE's Plan screen | `Plan` |
...
Prototype database import (`yam migrate --from-ade <path>`, P2) maps the prototype's electron-db records: `project` → `yam.config.yaml` (name, browser, threadCount → workers, url → baseUrl, takeStepScreenshot → screenshots), `flows` → `flows/<filename>` then v2→v3 migration, `project.locatorFile` JSON → `.locator` → seed bindings, `config.dataFile` JSON → `data.yaml`, `apirequests` → `api/<name>.yaml`. Results and images are not imported.
...
- The service's runtime (Draft 2.9). The ADE never runs the CLI with its own binary: the `RunAsNode` fuse is off in a packaged build, so `process.execPath` there is an application, not an interpreter, and the spawn produces a second ADE that prints no handshake. The runtime is resolved, in order, from `YAM_NODE`, then a `node` on `PATH` of the supported major or newer, then a Node binary shipped beside the CLI under `resources/` when the packager includes one; a packaged ADE that finds none renders the Project screen's alert naming the three places it looked and opens nothing. `yam surface doctor` and the ADE's own smoke check report which runtime was chosen. The smoke check (`YAM_ADE_SMOKE`) and `scripts/ade-smoke.mjs` run against the **packaged** application when one exists under `apps/ade/out/`, and against the unpackaged build otherwise, and say which.
- Opening a project without a dialog (Draft 2.9). `YAM_ADE_PROJECT=<dir>` opens that project on ready through the same `openProject` the Recent list uses; the desktop conformance gate passes the fixtures project this way, so its cases read a project screen rather than the welcome screen.
- Draft 2.10 absorptions. The preload bridge has a one-way `onServiceOpened` beside `onServiceLog`, because a project opened before the renderer exists has no caller to answer; it starts nothing and reads nothing. The packaged ADE carries a `pnpm deploy` of the CLI under `Resources/yam`, outside the asar, and a Node runtime is never shipped by default. On macOS a GUI application forked from a process outside the user's Aqua session gets no window, so the gate and the smoke check launch the packaged ADE through LaunchServices (`open -n --env …`). The ADE's Project screen names every button (LLD §16: names where the tree is thin); an unnamed interactive control in the ADE is a defect the desktop suite's snapshot case reports.
...
The ADE and the terminal cockpit `yam ui` are two renderers of one headless **screen model**, and both are views over the local service (§13.5) and nothing else. The approved mockups are under `docs/spec/design/` and are the visual reference; this section is the contract.
...
- **Language and adapters gain what the self suite needs.** Desktop adapters take `app.launch` (the executable or bundle, arguments, environment) and `app.quit` (a graceful route, then a signal) in configuration; the session opens by launching when no process of that name owns a window and closes by quitting; the language gains `Quit the app` (pattern 31, `action: "quit"`), which ends the session through the graceful route and fails if the process survives it. The Playwright adapter attaches to an existing Chromium when `YAM_CDP_URL` or `app.attach.cdpUrl` is set, exactly as the BiDi adapter attaches, so a flow can drive the ADE's renderer. The recorder grounds desktop trees through the same `snapshot()` and the fake gateway gains a desktop case set. Screenshots on macOS remain subject to the Screen Recording grant and are reported as `unreachable` without it, never as a failure.
...
- Desktop conformance target (P2): the Yam ADE itself, built from its repository in CI on Windows and macOS runners and launched with `YAM_A11Y=1`. The desktop conformance flows are: create a project, open a flow, run it, open the result, use the API client. No separate sample desktop app is built.
- Desktop healing cases (Draft 2.8, T6.1's "healing variant subset" made concrete): the ADE gains `YAM_A11Y_VARIANT=1|2`, where variant 1 renames one screen tab and one button on the Project screen and variant 2 moves the Record screen's gateway control into a different panel; a binding recorded at variant 0 must relocalize on both through the desktop adapter with the same weights and threshold as the web healing eval, and the desktop conformance report records the outcome per case.
...
- Draft 2.7 (after Phase 5 verification): `Step.guard.target` (§3.2); compensating-story steps keep their own statuses (§8.3); `yam trajectory compile` in the command table (§15); the trajectory line shape and the proposal context hash (§13.4); `POST /record` gateway, 409, and decision deadline (§13.5); the ADE Record screen chooses the gateway and renders failures (§13.6).
...
- Draft 2.11 (builder surfaces, after the owner's design review): one headless screen model rendered by the ADE and by `yam ui`, the sidebar-workspace-inspector architecture, the command palette with action parity across palette, TUI, SDK and CLI, the design system on Radix with the approved tokens, the accessibility contract, `--json` for agents (§13.7); `@svatah/yam-sdk` generated from the OpenAPI description with Python and Java clients (§13.8); the approved mockups under `docs/spec/design/`.
...
- Draft 2.9 (after Phase 7 verification): the ADE never runs the CLI with its own binary, resolves a Node runtime and says which, opens `YAM_ADE_PROJECT` on ready, and its smoke check runs against the packaged application (§13.6); the §7.5 budget is satisfied only by a measured snapshot of the project screen, whole-window reads or a native helper, the caller's deadline honoured, empty cases skipped and healing cases run at their own variant (§7.5); `dialog` steps arm the next dialog, with `W_DIALOG_UNARMED`, `W_DIALOG_NEVER_OPENED`, and an audit line for an unarmed default (§3.2, §4.2).
