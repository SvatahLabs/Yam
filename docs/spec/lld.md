...
- The budget is measured, not extrapolated (Draft 2.9). The number that satisfies this section is a snapshot of the ADE's project screen with a project open, taken by the gate and written into the report; a per-node cost measured on any other tree is not evidence, because a Chromium tree's containers and leaves cost differently. Reads that walk container by container do not meet the budget on that screen (measured: 51–55 ms per node, about 25 s for 488 nodes); a conforming bridge reads the whole window in a fixed number of events, for example `properties of every UI element of entire contents of window 1` and one further event per optional attribute, or uses a native helper that reads `AXUIElement` directly when the process-based bridge cannot reach 10 s. The bridge honours the caller's deadline; a script deadline shorter than the caller's is a defect. A conformance case with no checks is reported as skipped, not failed; a healing case is run only at the variant it is about.
