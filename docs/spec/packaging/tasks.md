...
- [x] **P2.4** The app's "Connect an agent" block emits the new command, and
...
- [~] **P3.1** Install the published tarball into an empty directory with no
  ...
  `PK-N1` — **the packing half is done and the installing half is not.** `npm
  pack --dry-run` measures the package's own contents, which is the half a
  registry cannot change; the other half needs something published, and nothing
  is. That is the owner's call and not an oversight.
- [x] **P3.2** Measure the installed tree and fail over 12 MB. `PK-01`, `PK-N2`
- [~] **P3.3** Install the base plus `playwright` and prove a browser connect;
  ...
  `PK-03` — **proven by construction, not by installing.** The adapter records
  what it resolved from its own location and `test/driver-verdicts.test.ts`
  drives all three answers; what is missing is a machine that genuinely lacks
  the driver, which this one does not.
- [x] **P3.4** Three installables on one machine at one contract: the app, the
