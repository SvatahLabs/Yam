...
**F9 — The cockpit still draws past the right edge, and one cockpit test flakes.** On master, `tui-pty.test.ts › at 100 columns` fails every time, alone or in the suite: `a line of 104 characters on a 100-column terminal`. It passed in the worktree, whose project and temporary paths are longer, so the overflow depends on the content's length rather than the terminal's width: the layout's budget is not applied to every cell. This is the one red test on master after the merge, and it is Phase 11's first correction. Separately, `cockpit.test.tsx › runs renders without throwing` failed once in a full run with the frame still at `loading…` and passes alone; it waits a fixed time instead of polling. T11.7.
