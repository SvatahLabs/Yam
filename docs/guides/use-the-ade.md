...
The Automation Development Environment is an Electron client for the local
...
project directory is the only source of truth, and the ADE stores a theme, a
...
```bash
pnpm -r build
pnpm --filter @svatah/yam-ade start
```

The ADE spawns `yam serve` on the project you open, reads the bearer token
from its stdout, and talks to it over HTTP and an event stream. Set
`YAM_ADE_PROJECT=<dir>` to open a project at start, and `YAM_NODE` to name
...
name, because the ADE is also the desktop conformance target for the macOS
...
the ADE, the terminal and the SDK agree on every screen. The HTTP API they all
