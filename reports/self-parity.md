...
**100 percent agreement** over the 32 check(s) both sides reached. The gate passes only at 100 percent (REQ-SELF-2).

| | Yam | External |
|---|---|---|
| Checks reached | 34 of 52 | 50 of 52 |
| Wall time | 346.7 s | 519.9 s |
...
| `cockpit.json-is-the-model` | external | Yam can drive a terminal since T22, and this still is not a flow: `yam ui --json` is the SDK's own answer and comparing it with the screen model ten times over is a program. What is missing is a sentence, not an adapter. |
...
| `cockpit.pseudo-terminal` | pass | pass | `yam ui` draws its panes in a real pseudo-terminal and its `--json` equals the model's state. |
...
|---|---|---|---|
| `app-playwright` | `npx playwright test --reporter=json` | 17.3 s | 38 name(s) |
| `artboards` | `node scripts/audit-artboards.mjs` | 0.8 s | 1 name(s) |
| `axe-sheet` | `node scripts/audit-sheet.mjs` | 0.6 s | 1 name(s) |
| `cli-vitest` | `npx vitest run test/front-door.test.ts test/check.test.ts test/help.test.ts test/explore.test.ts --reporter=json` | 3.5 s | 291 name(s) |
| `client-smoke` | `node scripts/smoke-clients.mjs` | 23.9 s | 1 name(s) |
| `desktop-gate` | `node scripts/desktop-conformance.mjs --adapter ax --report reports/adapter-ax.md` | 105.1 s | 1 name(s) |
| `front-door-check` | `node scripts/front-door-self.mjs check` | 37.4 s | 1 name(s) |
| `front-door-explore` | `node scripts/front-door-self.mjs explore` | 1.4 s | 1 name(s) |
| `front-door-help` | `node scripts/front-door-self.mjs help` | 0.9 s | 1 name(s) |
| `front-door-status` | `node scripts/front-door-self.mjs status` | 1.8 s | 1 name(s) |
| `healing-eval` | `node scripts/eval-healing.mjs --report reports/eval-healing.md` | 214.6 s | 1 name(s) |
| `tree-agreement` | `node scripts/tree-agreement.mjs` | 30.4 s | 1 name(s) |
| `tui-pty` | `npx vitest run test/tui-pty.test.ts --reporter=json` | 82.2 s | 78 name(s) |
| `yam` | `node packages/cli/dist/bin.js run evals/self --host none` | 225.5 s | 25 name(s) |
| `yam-cdp` | `node packages/cli/dist/bin.js run evals/self/cdp --host none` | 1.2 s | 1 name(s) |
| `yam-on-yam` | `node evals/self/yam-on-yam/run.mjs` | 120.0 s | 46 name(s) |
